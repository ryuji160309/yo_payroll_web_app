...
(function setupLandscapeMode() {
  ...
  const LANDSCAPE_CLASS = 'landscape-mode';
  const TITLE_ATTR = 'data-header-title-source';
  const EVENT_NAME = 'app:landscapemodechange';
  ...
  const resolveTitleSource = () => {
    const candidate = document.querySelector(`[${TITLE_ATTR}]`);
    if (candidate === titleSource) {
      return titleSource;
    }
    disconnectObserver();
    titleSource = candidate || null;
    if (titleSource) {
      ...
      titleObserver.observe(titleSource, {
        childList: true,
        characterData: true,
        subtree: true
      });
  ...
  function updateHeaderTitle() {
    ...
    const source = resolveTitleSource();
    if (!currentLandscape) {
    ...
    headerTitleEl.hidden = text === '';
  }

  function applyLandscapeState(nextLandscape) {
    ...
    currentLandscape = nextLandscape;
    document.documentElement.classList.toggle(LANDSCAPE_CLASS, nextLandscape);
    if (document.body) {
      document.body.classList.toggle(LANDSCAPE_CLASS, nextLandscape);
    }
    if (nextLandscape) {
      updateHeaderTitle();
    } else if (headerTitleEl) {
    ...
    try {
      window.dispatchEvent(new CustomEvent(EVENT_NAME, {
        detail: { landscape: nextLandscape }
      }));
    } catch (error) {
  ...
  const evaluateOrientation = () => {
    ...
    applyLandscapeState(nextLandscape);
  };
