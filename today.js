    ...
    if (!section.employees || section.employees.length === 0) {
      ...
      section.employees.forEach((employee, employeeIndex) => {
        employee.segments.forEach(seg => {
          ...
          const isActive = nowMinutes !== null && nowMinutes >= seg.start && nowMinutes < seg.end;
          block.className = `shift-block${isActive ? ' shift-block--active' : ''}`;
          block.style.top = minutesToPercent(seg.start);
          block.style.height = `calc(${minutesToPercent(seg.end)} - ${minutesToPercent(seg.start)})`;
          block.style.setProperty('--stack-index', employeeIndex % 3);
          ...
          const label = document.createElement('div');
          label.className = 'shift-block__time';
